<script lang="ts" setup>
    ...
    const xpBadgeClass = computed(() =>
        twMerge('text-xs font-bold px-3 py-1 rounded-full', xpBadgeClasses[props.variant])
    );
    const attributeBadgeClass = computed(() =>
        twMerge('text-xs font-bold px-3 py-1 rounded-full', attributeBadgeClasses[props.variant])
    );
</script>
...
<template>
    <div :class="containerClasses">
        ...
        <div class="flex items-center gap-2">
            <span :class="xpBadgeClass">{{ xpLabel }}</span>
            <span :class="attributeBadgeClass">{{ attributeLabel }}</span>
        </div>
    </div>
</template>
