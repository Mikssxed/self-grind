...
SelfGrind/
├── src/
│   ├── SelfGrind.API/               ← HTTP layer (controllers, middleware)
│   ├── SelfGrind.Application/       ← CQRS handlers, validators, AutoMapper
│   ├── SelfGrind.Domain/            ← Entities, exceptions, repository interfaces
│   ├── SelfGrind.Infrastructure/    ← EF Core, repositories, JWT, email
...
└── .claude/                         ← Claude context, skills, templates
    ├── context/
    ├── skills/
    └── templates/
```
...
```bash
# Backend — from src/SelfGrind.API
dotnet run                  # start API on http://localhost:5081
dotnet watch run            # with hot reload

# Frontend — from src/SelfGrind.App
...
| `src/SelfGrind.API/Middlewares/ErrorHandlingMiddleware.cs` | Exception → HTTP mapping |
